...
question("1. Would you say this data is in tidy format?",
...
family_data <-
  tibble(
    wave = c(rep(1, 100), rep(2, 100)),
    communicationscore = 0,
    satisfactionscore = 0,
    groupid = rep(1:100, each = 2),
    member = rep(c("child","parent"), 100)
  )

# Simulating scores
for(i in 1:200){
  # Children
  if(family_data[i,5] == "child"){
    # Wave 1
    if(family_data[i,1] == 1){
      family_data[i,2] <- family_data[i,2] + rnorm(1, mean = 5, sd = 1)
      family_data[i,3] <- family_data[i,2] + rnorm(1, mean = 5, sd = 1)
    }
    # Wave 2
    else{
      family_data[i,2] <- family_data[i,2] + rnorm(1, mean = 6, sd = 1)
      family_data[i,3] <- family_data[i,2] + rnorm(1, mean = 6, sd = 1)
    }
  }
  # Parent
  else{
    ...
    # Wave 2
      ...
      family_data[i,2] <- family_data[i,2] + rnorm(1, mean = 7, sd = 1)
      family_data[i,3] <- family_data[i,2] + rnorm(1, mean = 7, sd = 1)
    }
  }
}
...
fam_data <- family_data %>% 
  pivot_wider(
    id_cols = c(wave, groupid, member),
    names_from = member,
    names_sep = "_",
    values_from = c(communicationscore, satisfactionscore)
  )
